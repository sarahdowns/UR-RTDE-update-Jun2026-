...
def main():
    ...
    try:
        ...
        if args.home:
            ...
            # Move Master Arm to its home pose
            execute_smooth_move(master_arm, "Master", HOME_MASTER_POSE)

            # Wait
            print("\n--- Waiting between moves ---")
            time.sleep(1)

            # Move Slave Arm to its home pose
            execute_smooth_move(slave_arm, "Slave", HOME_SLAVE_POSE)
            ...
            # Move Master Arm to its target pose
            execute_smooth_move(master_arm, "Master", DESTINATION_MASTER_POSE)

            # Wait for 5 seconds
            print("\n--- Waiting between moves ---")
            time.sleep(1)

            # Move Slave Arm to its target pose
            execute_smooth_move(slave_arm, "Slave", DESTINATION_SLAVE_POSE)
