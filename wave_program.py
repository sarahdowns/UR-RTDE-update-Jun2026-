...
def execute_simultaneous_wave(master_arm, slave_arm):
    """
    Gets the robots' current positions and executes a simultaneous wave motion.

    Args:
        master_arm: The URRobot object for the master arm.
        slave_arm: The URRobot object for the slave arm.
    """
    print("\n--- Waving Both Arms Simultaneously ---")

    # 1. Get the initial positions of both arms.
    print("[Step 1] Reading initial joint positions for both arms...")
    initial_master_joints = np.array(master_arm.getj())
    initial_slave_joints = np.array(slave_arm.getj())
    print(f"✓ Master Arm initial positions: {np.round(initial_master_joints, 2)}")
    print(f"✓ Slave Arm initial positions: {np.round(initial_slave_joints, 2)}")

    # 2. Generate the wave trajectories for each arm.
    print("[Step 2] Generating wave trajectories...")
    master_wave_path = generate_wave_path(initial_master_joints)
    slave_wave_path = generate_wave_path(initial_slave_joints)
    print("✓ Trajectories generated.")

    # 3. Execute the wave simultaneously, waypoint by waypoint.
    print("!!! BOTH ARMS WILL NOW WAVE !!!")
    
    # Ensure paths have the same number of points to keep them in sync.
    if len(master_wave_path) != len(slave_wave_path):
        print("✗ Error: Wave paths have different lengths. Aborting.")
        return

    for i in range(len(master_wave_path)):
        master_target = master_wave_path[i]
        slave_target = slave_wave_path[i]
        print(f"  -> Moving to waypoint {i+1}/{len(master_wave_path)}...")

        # Send non-blocking commands to start both moves at the same time.
        master_arm.movej(master_target, acc=WAVE_ACCELERATION, vel=WAVE_VELOCITY, wait=False)
        slave_arm.movej(slave_target, acc=WAVE_ACCELERATION, vel=WAVE_VELOCITY, wait=False)

        # Wait for both moves to complete before proceeding to the next waypoint.
        # This uses an internal wait method, which is suitable for this synchronization.
        # A timeout is set to prevent getting stuck indefinitely.
        master_arm._wait_for_move(target=master_target, joints=True, timeout=5)
        slave_arm._wait_for_move(target=slave_target, joints=True, timeout=5)

    print("✓ Simultaneous wave complete.")
...
def main():
    ...
    try:
        ...
        # Execute the simultaneous wave
        execute_simultaneous_wave(master_arm, slave_arm)
